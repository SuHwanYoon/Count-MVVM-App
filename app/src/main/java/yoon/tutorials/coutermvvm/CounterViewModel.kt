...
@HiltViewModel
// CounterRepository를 주입받음
class CounterViewModel @Inject constructor
    (private val _repository: CounterRepository) : ViewModel() {
    // ViewModel에서 상태를 관리하기 위한 MutableState
    private val _count = mutableStateOf(_repository.getCounter().count)
    // ViewModel에서 상태를 조회하기 위한 State
    // 외부에서 count를 통해 상태를 조회할 수 있지만, 직접 변경할 수는 없습니다.
    val count: State<Int> = _count

    // View에서 접근하는 ViewModel의 증가 함수
    fun increment() {
    // 1. 증가된 Model data값을 repository 접근함수를 사용해서 1차적으로 Model Instance에 접근
    // 2. Model Instance에 접근후 증가된 Instance의 property에 접근
    // 3. 증가된 Instance의 property 값을 ViewModel의 private State변수에 담기
        _repository.incrementCounter()
        _count.value = _repository.getCounter().count
    }
    // View에서 접근하는 ViewModel의 감소 함수
    fun decrement() {
        // 1. 감소된 Model data값을 repository 접근함수를 사용해서 1차적으로 Model Instance에 접근
        // 2. Model Instance에 접근후 감소된 Instance의 property에 접근
        // 3. 감소된 Instance의 property 값을 ViewModel의 private State변수에 담기
